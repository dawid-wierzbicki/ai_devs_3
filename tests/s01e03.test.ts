...
describe('S01E03 - Big Model Limits Task Integration Test', () => {
  test('should run s01e03 script and get code -1004 response', async () => {
    ...
    const runScript = (): Promise<{ stdout: string; stderr: string; code: number | null }> => {
      return new Promise((resolve) => {
        ...
        const child = spawn('npm', ['run', 'dev:limits'], {
          stdio: 'pipe',
          cwd: process.cwd(),
          env: {
            ...process.env,
