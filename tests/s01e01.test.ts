...
describe('S01E01 - Prove You\'re Not Human Task Integration Test', () => {
  test('should run s01e01 script and get code -1004 response', async () => {
    ...
    const runScript = (): Promise<{ stdout: string; stderr: string; code: number | null }> => {
      return new Promise((resolve) => {
        ...
        const child = spawn('npm', ['run', 'dev:prove'], {
          stdio: 'pipe',
          cwd: process.cwd(),
          env: {
            ...process.env,
    ...
    const hasCode1004 = result.stdout.includes('SUCCESS! Found flag:');
